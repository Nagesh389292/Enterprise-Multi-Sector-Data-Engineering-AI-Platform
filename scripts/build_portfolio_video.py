...
print("=== STEP 1: GENERATING 8.5-MINUTE STORY-DRIVEN AI VOICE NARRATION ===")
speaker = win32com.client.Dispatch('SAPI.SpVoice')
speaker.Rate = -1 # Professional, deliberate pacing for technical presentation

stream = win32com.client.Dispatch('SAPI.SpFileStream')
stream.Open(audio_path, 3, False)
speaker.AudioOutputStream = stream

portfolio_narration_script = """
...
In the Data Engineering core, raw data enters Bronze storage attached with ingestion timestamps, metadata provenance, and UUID primary keys. The Silver stage enforces schema validation rules; records failing quality assertions are isolated in data quarantine for compliance auditing. Gold data marts aggregate domain metrics including credit card fraud scores, banking default risks, hospital bed occupancy rates, clinical readmission risks, insurance fraud probabilities, and retail gross revenue totals.
...
speaker.Speak(portfolio_narration_script)
stream.Close()

with wave.open(audio_path, 'rb') as w:
    audio_dur = w.getnframes() / float(w.getframerate())

print(f"[Audio Generator] Generated WAV Audio: {audio_path} ({audio_dur:.3f} seconds / {audio_dur/60:.2f} minutes)")

print("=== STEP 2: GENERATING SYNCHRONIZED MP4 VIDEO WITH EMBEDDED AAC AUDIO ===")

# 11 Storyboard Sections with proportional duration scaling matching audio_dur
sections = [
    ("01. INTRODUCTION & BUSINESS CONTEXT", "architecture/platform_architecture_diagram.png", 35.0),
    ("02. END-TO-END SYSTEM TOPOLOGY", "architecture/platform_architecture_diagram.png", 55.0),
    ("03. PYSPARK MEDALLION LAKEHOUSE (BRONZE -> SILVER -> GOLD)", "cicd/master_tests_71_pass.png", 65.0),
    ("04. DATABRICKS DELTA LAKE 6/6 SECTOR RECONCILIATION", "databricks/reconciliation_report.png", 50.0),
    ("05. MLOPS & PREDICTIVE RISK SUITE", "ai/llm_gateway_architecture.png", 50.0),
    ("06. MULTI-TIER AI COPILOT & AST SQL SECURITY", "ai/llm_gateway_architecture.png", 65.0),
    ("07. APACHE SUPERSET BI DASHBOARDS", "dashboards/superset_executive_logged_in.png", 52.0),
    ("08. REACT COMMAND CENTER WEB APPLICATION", "frontend/react_command_center_ui.png", 40.0),
    ("09. MASTER CI/CD PIPELINE & TESTING (71/71 PASS)", "cicd/master_tests_71_pass.png", 30.0),
    ("10. GOOGLE CLOUD PLATFORM TERRAFORM BOUNDARY", "gcp/gcp_console_project.png", 23.0),
    ("11. SUMMARY & OFFICIAL ARCHITECTURE CLASSIFICATION", "architecture/platform_architecture_diagram.png", 25.0)
]
...
print(f"[FFmpeg Video Encoder] Encoding 25fps H.264 video + AAC audio (Exact Duration: {audio_dur:.3f}s)...")
...
print(f"[FFmpeg Video Encoder] Exit code: {res.returncode}")
...
if os.path.exists(final_video_path):
    ...
    print(f"[FFmpeg Video Encoder] SUCCESS! Final Muxed MP4: {final_video_path} ({size_mb:.2f} MB)")
else:
    print(f"[FFmpeg Video Encoder] Error: {res.stderr}")
